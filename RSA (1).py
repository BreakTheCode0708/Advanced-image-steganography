...
from DS import rsakeys,sign,verify
import math
...
# encrypts a plaintext message using the current key
    global plaintext, numC, j, X,H,sum1
    X=[]
    sum1=0
    ...
    pr1,pk1= rsakeys()
    H=sign(pr1,sum1)
    print("the generated signature :",H)
...
print("/nTo encrypt a message with the current key, type: 'Encrypt'")
...
while mm != 'quit':
    ...
    if mm.lower() == 'encrypt':
        ...
        kk=int(input("enter the key for decryption:"))
        if(kk==pr1):
            Decrypt()
        else:
