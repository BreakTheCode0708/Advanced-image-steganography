...
def fn(i, abs_height, abs_width, pix):
    ...
    for k in range(i, abs_height - 1):
        ...
        for j in range(n):
            if temp:
                pix[k, j], pix[k + (abs_height - i) // 2 + (abs_height - i) % 2, j + n + abs_width % 2] = pix[k + (
                            abs_height - i) // 2 + (abs_height - i) % 2, j + n + abs_width % 2], pix[k, j]
            else:
                pix[k, j], pix[k - (abs_height - i) // 2 + (abs_height - i) % 2, j + n + abs_width % 2] = pix[k - (
                            abs_height - i) // 2 + (abs_height - i) % 2, j + n + abs_width % 2], pix[k, j]
...
for i in range(height):
    ...
    for j in range(width // 2):
        if temp:
            pix[i, j], pix[i + height // key, j + width // 2 + width % 2] = pix[
                                                                                i + height // key, j + width // 2 + width % 2], \
                                                                            pix[i, j]
        else:
            pix[i, j], pix[i - height // key, j + width // 2 + width % 2] = pix[
                                                                                i - height // key, j + width // 2 + width % 2], \
